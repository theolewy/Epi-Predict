...
def preprocess_data(
    ...
    df = pd.read_csv(
        data_path, parse_dates=["Date"], infer_datetime_format=True, dayfirst=True
    ).set_index(["Area", "Date"])
    ...
    return PreprocessedData(
        Rs, Ds, CMs, new_cases, new_deaths, active_cms, Cs, unique_Cs, C_indices, n_days_nn_input, num_percentiles
    )

def summarise_some_data(data, num_percentiles):
    # data inputted has dim regions x time or samples x region x time
    ...
    if data.ndim == 2:
        ...
        summary = np.zeros((nRs,2+num_percentiles))

        std = np.std(data, axis=-1)
        slope = (np.mean(data[:,-3:], axis=-1) - np.mean(data[:, :3], axis=-1))/nDs
        
        percentile_jump = 100/(num_percentiles+1)

        for i in range(num_percentiles):
                    
            percentile = np.percentile(data, percentile_jump*i, axis=-1) 
            summary[:,i]
        ...
        summary = np.zeros((nSs, nRs, 2+num_percentiles))
...
class PreprocessedData(object):
    ...
    def __init__(
        self, Rs, Ds, CMs, new_cases, new_deaths, active_cms, Cs, unique_Cs, C_indices, n_days_nn_input, num_percentiles
    ):
        """
        ...
        self.scale_data()
        self.get_staggered_matrices(n_days_nn_input)
        self.summarise_data(n_days_nn_input, num_percentiles)
    ...
    def moving_averages(self):
        self.moving_average_cases = np.zeros((self.nRs, self.nDs))
        self.moving_average_deaths = np.zeros((self.nRs, self.nDs))

        for i in range(6, self.nDs):
            self.moving_average_cases[:, i] = jnp.sum(self.new_cases[:,i-7:i], axis=1)/7           
            self.moving_average_deaths[:, i] = jnp.sum(self.new_deaths[:,i-7:i], axis=1)/7
        
        self.moving_average_cases[:,:7] = self.new_cases[:,:7]
        self.moving_average_deaths[:,:7] = self.new_deaths[:,:7]
        
    def scale_data(self):

        # we use log data so that feature scaling works better
        raw_cases_log = jnp.log(1+self.new_cases.data ) 
        raw_deaths_log = jnp.log(1+self.new_deaths.data) 
        
        ave_cases_log = jnp.log(1+self.moving_average_cases) 
        ave_deaths_log = jnp.log(1+self.moving_average_deaths)

        #feature scale
        mean_cases = jnp.mean(raw_cases_log, axis=1)
        std_cases = jnp.std(raw_cases_log, axis=1)
        mean_deaths = jnp.mean(raw_deaths_log, axis=1)
        std_deaths = jnp.std(raw_deaths_log, axis=1)
        ...
        self.cases_mean = mean_cases_expand
        self.cases_std = std_cases_expand
        self.deaths_mean = mean_deaths_expand
        self.deaths_std = std_deaths_expand

        self.new_cases_scaled =  (raw_cases_log - mean_cases_expand)/std_cases_expand
        self.new_deaths_scaled =  (raw_deaths_log - mean_deaths_expand)/std_deaths_expand
        
        self.moving_average_cases_scaled =  (ave_cases_log - mean_cases_expand)/std_cases_expand
        self.moving_average_deaths_scaled =  (ave_deaths_log - mean_deaths_expand)/std_deaths_expand
        
    def get_staggered_matrices(self, n_days_nn_input=21):
                
        def stagger_data_into_matrix(data, n_days_nn_input):
            ...
            _, nn_input = jax.lax.scan(
                stagger_matrix_func,
                data[:, :n_days_nn_input],
                data[:, n_days_nn_input:].transpose()
            )

            nn_input = nn_input.transpose((1,2,0))
            nn_input = jnp.concatenate((jnp.expand_dims(data[:, :n_days_nn_input],2), nn_input), axis=-1)
            nn_input = jnp.transpose(jnp.expand_dims(nn_input, 2), (0, 3, 2, 1))

            return nn_input
        
        self.cases_nn_input = stagger_data_into_matrix(self.new_cases_scaled , n_days_nn_input)
        self.deaths_nn_input = stagger_data_into_matrix(self.new_deaths_scaled , n_days_nn_input)

        self.moving_ave_cases_nn_input = stagger_data_into_matrix(self.moving_average_cases_scaled , n_days_nn_input)
        self.moving_ave_deaths_nn_input = stagger_data_into_matrix(self.moving_average_deaths_scaled , n_days_nn_input)
        
    

    def summarise_data(self, n_days_nn_input=21, num_percentiles=9):
        
        if num_percentiles is None:
            num_percentiles = 1
        
        summarised_cases = np.zeros((self.nRs, self.nDs-n_days_nn_input+1, num_percentiles+2))
        summarised_deaths = np.zeros((self.nRs, self.nDs-n_days_nn_input+1, num_percentiles+2))
        
        summarised_moving_ave_cases = np.zeros((self.nRs, self.nDs-n_days_nn_input+1, num_percentiles+2))
        summarised_moving_ave_deaths = np.zeros((self.nRs, self.nDs-n_days_nn_input+1, num_percentiles+2))

        for i in range(n_days_nn_input, self.nDs+1):
            
            case_summary = summarise_some_data(self.new_cases_scaled[:,i-n_days_nn_input:i], num_percentiles)
            death_summary = summarise_some_data(self.new_deaths_scaled[:,i-n_days_nn_input:i], num_percentiles)
            
            case_moving_ave_summary = summarise_some_data(self.moving_average_cases_scaled[:,i-n_days_nn_input:i], num_percentiles)
            death_moving_ave_summary = summarise_some_data(self.moving_average_deaths_scaled[:,i-n_days_nn_input:i], num_percentiles)
            
            summarised_cases[:,i-n_days_nn_input,:] = case_summary
            summarised_deaths[:,i-n_days_nn_input,:] = death_summary
            
            summarised_moving_ave_cases[:,i-n_days_nn_input,:] = case_moving_ave_summary
            summarised_moving_ave_deaths[:,i-n_days_nn_input,:] = death_moving_ave_summary

        
        self.new_cases_summary = np.expand_dims(summarised_cases, axis=2)
        self.new_deaths_summary = np.expand_dims(summarised_deaths, axis=2)

        self.new_cases_moving_ave_summary = np.expand_dims(summarised_moving_ave_cases, axis=2)
        self.new_deaths_moving_ave_summary = np.expand_dims(summarised_moving_ave_deaths, axis=2)
